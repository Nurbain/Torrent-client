//Projet de RP S5-2016 Nathan URBAIN , Marek Felsöci
...
int main(int argc , char **argv)
{
  ...
  addrlgn         = sizeof(struct sockaddr_in6);
	...
	//Recupration du type de message cf figure 2 du sujet
	unsigned char msg_type;
	if(strcmp(argv[4], "put" ) ==0)
	{
		msg_type = 110; //type put
	}

	if (strcmp(argv[4] , "get") == 0)
	{
		msg_type = 112; // type get
	}
	...
	memcpy(buf, &msg_type, sizeof(msg_type)); //type du  message
	memcpy(buf+sizeof(msg_type), &msg_lgn , sizeof(msg_lgn)); // taille du message
	memcpy(buf+sizeof(msg_type)+sizeof(msg_lgn), &hash_type , sizeof(hash_type)); //type de hash
	memcpy(buf+sizeof(msg_type)+sizeof(msg_lgn)+sizeof(hash_type) , &hash_lgn , sizeof(hash_lgn)); // taille de hash
	memcpy(buf+sizeof(msg_type)+sizeof(msg_lgn)+sizeof(hash_type)+sizeof(hash_lgn), hash , sizeof(hash)); //hash
	memcpy(buf+sizeof(msg_type)+sizeof(msg_lgn)+sizeof(hash_type)+sizeof(hash_lgn)+sizeof(hash), &client_type , sizeof(client_type)); // type de client
	memcpy(buf+sizeof(msg_type)+sizeof(msg_lgn)+sizeof(hash_type)+sizeof(hash_lgn)+sizeof(hash)+sizeof(client_type), &client_lgn , sizeof(client_lgn)); // taille de client
	memcpy(buf+sizeof(msg_type)+sizeof(msg_lgn)+sizeof(hash_type)+sizeof(hash_lgn)+sizeof(hash)+sizeof(client_type)+sizeof(client_lgn), &client_port , sizeof(client_port)); //port du client
	memcpy(buf+sizeof(msg_type)+sizeof(msg_lgn)+sizeof(hash_type)+sizeof(hash_lgn)+sizeof(hash)+sizeof(client_type)+sizeof(client_lgn)+sizeof(client_port), & client_addr , sizeof(client_addr)); //adresse du client
	...
	memcpy(&pclient_port , buf+sizeof(msg_type)+sizeof(msg_lgn)+sizeof(hash_type)+sizeof(hash_lgn)+sizeof(hash)+sizeof(client_type)+sizeof(client_lgn), sizeof(pclient_port)); //port du client
	memcpy(&pclient_addr , buf+sizeof(msg_type)+sizeof(msg_lgn)+sizeof(hash_type)+sizeof(hash_lgn)+sizeof(hash)+sizeof(client_type)+sizeof(client_lgn)+sizeof(pclient_port), sizeof(pclient_addr)); //adresse du client
	...
	my_addr.sin6_family = AF_INET6; // domaine d'adresse
	my_addr.sin6_port = htons(atoi(argv[3]));	//port
	my_addr.sin6_addr = in6addr_any; //on ecoute sur une adresse quelconque
	addrlgn2 = sizeof(struct sockaddr_in6);



	//association socket a l'adresse
	if (bind(sockfd2, (struct sockaddr *) &my_addr, addrlgn2)== -1)
	{
		perror("bind fail \n");
		close(sockfd2);
		exit(EXIT_FAILURE);
	...
	//Affichage des différentes données pour verification
		...
		while (rmsg_lgn > 89)
		{
			...
			memcpy(&rclient_port, s+ack+sizeof(msg_type)+sizeof(msg_lgn)+sizeof(hash_type)+sizeof(hash_lgn)+sizeof(hash)+sizeof(client_type)+sizeof(client_lgn), sizeof(pclient_port)); //port client
			memcpy(&rclient_addr,s+ack+sizeof(msg_type)+sizeof(msg_lgn)+sizeof(hash_type)+sizeof(hash_lgn)+sizeof(hash)+sizeof(client_type)+sizeof(client_lgn)+sizeof(client_port),sizeof(pclient_addr)); //adresse client
			s+=21;
			rmsg_lgn-=21;
